...
On the move to the final state, we'll need to implement functions which allows agent respond to the commands with action and its associted objects included.  Items will be placed randomly on the maze; we'll return their coordinates and agent's coordinate to compute the shortest paths for the agent to quickly get to the item listed in the user's command. 

In case the inventory is full, we may either decide to craft some valid items to create other item (to retreive rewards) or just simply discard a couple last items in the inventory.  This idea is similar to what we have done in homework 2 or the same as shown in the "craft_work" file in the example folders.  This is the plan and we'll be implementing it as soon as we move on to the next stage after the status report.
